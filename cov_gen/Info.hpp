...
namespace ArchCov
{
  ...
  public:
    ...
    typedef std::pair<std::string, Enum> enumType;
    enum RESOLVE : uint32_t { COALESCE, SEPARATE, NONE };
    typedef typename std::vector<enumType> enumBins;

    typedef std::tuple<std::string, Attribute, RESOLVE> attType;
    typedef typename std::vector<attType> attBins;
    ...
    void points(enumBins& enums, attBins& atts, fieldsBins& fields, instBins& insts, csrBins& csrs) const;
